...
export function LeafletMap({
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      map = L.map(divRef.current!, { zoomControl: true })
  ...
  // Update primary marker position when it changes externally
  useEffect(() => {
    const primary = markers.find((m) => m.primary);
    if (primary && markerRef.current) {
  ...
  return (
    <div
      ref={divRef}
      style={{ width: '100%', height: '280px', borderRadius: 'var(--radius)', ...style }}
    />
  );
}
